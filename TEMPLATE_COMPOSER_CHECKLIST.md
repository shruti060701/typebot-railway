...
| `GITHUB_CLIENT_ID` | Your GitHub OAuth App's Client ID | **Yes**, but only if using GitHub login | GitHub OAuth App client ID. Create one at github.com/settings/developers — Authorization callback URL must be `<your-domain>/api/auth/callback/github`. |
| `GITHUB_CLIENT_SECRET` | Your GitHub OAuth App's Client Secret | **Yes**, but only if using GitHub login | GitHub OAuth App client secret. Used together with `GITHUB_CLIENT_ID` to enable "Continue with GitHub" login — this project uses GitHub OAuth instead of SMTP/email auth since Gmail is confirmed not to work on Railway (per the official Typebot Railway template's own notes) and no other SMTP provider was configured. |
